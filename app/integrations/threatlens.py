...
class ThreatLensAlertResponse(BaseModel):
    ...
    def identifier(self) -> str:
        ...
        if not identifier:
            raise ThreatLensMalformedResponseError("alert identifier missing")
        return identifier
...
class ThreatLensClient:
    ...
    def fetch_alert(self, alert_id: str) -> ThreatLensAlertResponse:
        ...
        if base.scheme not in {"http", "https"} or not base.netloc:
            raise ThreatLensError("ThreatLens is not configured with a valid HTTP(S) base URL")
        if not alert_id or any(char not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_.:-" for char in alert_id):
        ...
        connection = connection_type(base.netloc, timeout=self.settings.threatlens_connect_timeout_seconds)
        path = f"{base.path.rstrip('/')}/v1/alerts/{quote(alert_id, safe='._:-')}"
        if base.query:
            path = f"{path}?{base.query}"
        headers = {"Accept": "application/json"}
        if self.settings.threatlens_api_key:
            headers["Authorization"] = f"Bearer {self.settings.threatlens_api_key}"
        try:
            connection.request("GET", path, headers=headers)
            response = connection.getresponse()
            if response.fp is not None and hasattr(response.fp, "raw") and hasattr(response.fp.raw, "_sock"):
                response.fp.raw._sock.settimeout(self.settings.threatlens_read_timeout_seconds)
            ...
            if result.identifier != alert_id:
                raise ThreatLensMalformedResponseError("alert identifier does not match request")
            return result
    ...
    def to_investigation_request(self, alert: ThreatLensAlertResponse) -> InvestigationRequest:
        metadata = dict(alert.metadata)
        raw_event = dict(alert.raw_event)
        asset_data = alert.asset or raw_event.get("asset") or metadata.get("asset") or {}
        asset = Asset.model_validate({
            "hostname": asset_data.get("hostname") or metadata.get("hostname"),
            "ip": asset_data.get("ip") or metadata.get("ip"),
            "url": asset_data.get("url") or metadata.get("url"),
        }) if isinstance(asset_data, dict) else None
        evidence = dict(raw_event)
        evidence.setdefault("threatlens_metadata", metadata)
        finding_data: dict[str, Any] = {
            "type": alert.category or raw_event.get("type") or "unknown",
            "title": raw_event.get("title") or alert.category or "ThreatLens alert",
            "severity": alert.severity.value,
            "evidence": evidence,
            "category": alert.category,
        }
        for key in ("port", "protocol", "url", "ip"):
            if key in raw_event:
        ...
        return InvestigationRequest(
            ...
            mitre_attack=raw_event.get("mitre_attack", metadata.get("mitre_attack", [])) or [],
        )
